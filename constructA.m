function [A, elapse] = constructA(fea,options)
%	Usage:
%	A = constructA(fea,options)
%
...
%                  
%           NeighborMode -  Indicates ho to construct the graph. Choices
%                           are: [Default 'KNN']
%                'KNN'            -  k = 0
%                                       Complete graph
%                                    k > 0
%                                      Put an edge beteen two nodes if and
...
%      bSelfConnected  -   0 or 1. Indicates whether A(i,i) == 1. Default 1
...
if strcmpi(options.NeighborMode,'KNN') && (options.k > 0)
    if strcmpi(options.Metric,'Euclidean')
        ...
        A = sparse(G(:,1),G(:,2),G(:,3),nSmp,nSmp);
    else
        if ~options.bNormalized
            [nSmp, nFea] = size(fea);
            if issparse(fea)
    ...
    if isfield(options,'bSemiSupervised') && options.bSemiSupervised
        ...
        A(options.semiSplit,options.semiSplit) = (Asup>0)*options.SameCategoryWeight;
    end
